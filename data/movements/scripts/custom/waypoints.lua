...
function onStepIn(cid, item, toPos, itemEx, fromPos)
	...
	if isPlayer(cid) and match ~= -1 and getPlayerStorageValue(cid, storage) ~= 1 and getPlayerName(cid) ~= "Account Manager" and getPlayerStorageValue(cid, 102) ~= -1 then
		doSendMagicEffect(getPlayerPosition(cid), 29)
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "You've unlocked a new !travel destination, " .. waypoints[match].name)
		doPlayerSendTextMessage(cid, MESSAGE_LOOT, "You've unlocked a new !travel destination, " .. waypoints[match].name)
