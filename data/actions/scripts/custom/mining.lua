...
function onUse(cid, item, fromPos, itemEx, toPos)
	...
	if item.itemid ~= 1945 and item.itemid ~= 1946 then
		...
		local pos = deepcopy(toPos)
		pos.stackpos = 255
		pos.y = pos.y+1
		local usr_item = getThingFromPosition(pos)
		local chance = math.random(1000)
		local chance2 = math.random(1000)
		local itemType = 0
		if (usr_item.itemid == rusty.armor) then
			itemType = CONST_SLOT_ARMOR
		elseif (usr_item.itemid == rusty.legs) then
			itemType = CONST_SLOT_LEGS
		elseif (usr_item.itemid == rusty.feet) then
